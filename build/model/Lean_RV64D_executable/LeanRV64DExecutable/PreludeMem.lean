...
import LeanRV64DExecutable.ReadWriteV1
...
def write_ram (wk : write_kind) (app_1 : physaddr) (width : Nat) (data : (BitVec (8 * width))) (_meta : Unit) : SailM Bool := do
  ...
  | .Ok _ =>
    (let _ : Unit := (__WriteRAM_Meta addr width _meta)
    (pure true))
  | .Err () => (pure false)
...
/-- Type quantifiers: k_ex87774# : Bool, width : Nat, width ≥ 0, 0 < width ∧
