...
import LeanRV32DExecutable.RiscvRegs
import LeanRV32DExecutable.RiscvSysRegs
import LeanRV32DExecutable.RiscvPmpRegs
import LeanRV32DExecutable.RiscvExtRegs
import LeanRV32DExecutable.RiscvVextRegs
import LeanRV32DExecutable.RiscvVextControl
import LeanRV32DExecutable.RiscvSysExceptions
import LeanRV32DExecutable.RiscvZihpm
import LeanRV32DExecutable.RiscvSscofpmf
import LeanRV32DExecutable.RiscvZkrControl
import LeanRV32DExecutable.RiscvSmcntrpmf
import LeanRV32DExecutable.RiscvSysControl
import LeanRV32DExecutable.RiscvInstRetire
...
def read_CSR (b__0 : (BitVec 12)) : SailM (BitVec (2 ^ 2 * 8)) := do
...
def write_CSR (b__0 : (BitVec 12)) (value : (BitVec (2 ^ 2 * 8))) : SailM (BitVec (2 ^ 2 * 8)) := do
  bif (b__0 == (0x301 : (BitVec 12)))
  then
    (do
...
/-- Type quantifiers: k_ex136199# : Bool -/
def doCSR (csr : (BitVec 12)) (rs1_val : (BitVec (2 ^ 2 * 8))) (rd : regidx) (op : csrop) (is_CSR_Write : Bool) : SailM ExecutionResult := do
      ...
      then (pure (Ext_CSR_Check_Failure ()))
        ...
        (do
          ...
          let csr_val ← (( do
            bif is_CSR_Read
            then (read_CSR csr)
            else (pure (zeros (n := ((2 ^i 2) *i 8)))) ) : SailM xlenbits )
          bif is_CSR_Write
          then
            (do
