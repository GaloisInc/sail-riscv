...
import LeanRV32DExecutable.Common
...
import LeanRV32DExecutable.RiscvVmem
import LeanRV32DExecutable.RiscvInstsEnd
import LeanRV32DExecutable.RiscvStepCommon
import LeanRV32DExecutable.RiscvStepExt
import LeanRV32DExecutable.RiscvDecodeExt
import LeanRV32DExecutable.RiscvFetch
...
/-- Type quantifiers: k_ex151589# : Bool, step_no : Int -/
...
/-- Type quantifiers: k_ex151626# : Bool, step_no : Nat, 0 ≤ step_no -/
def try_step (step_no : Nat) (exit_wait : Bool) : SailM Bool := do
  ...
  | .Step_Pending_Interrupt (intr, priv) =>
    (do
      bif (get_config_print_instr ())
      then (print_bits_effect "Handling interrupt: " (interruptType_to_bits intr))
      else (pure ())
        ...
        let i ← (( do
          ...
          then
            (do
              ...
              bif (exit_val == 0)
              then (print_effect "SUCCESS")
              else (print_int_effect "FAILURE: " exit_val)
              (pure i))
          else
            (do
              let i : Nat := (i +i 1)
